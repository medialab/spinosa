...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier launch", () => {
    expect(hasExistingAppState([file("spinosa.settings")])).toBe(true)
    expect(hasExistingAppState([file("opencode.settings")])).toBe(true)
    expect(hasExistingAppState([file("spinosa.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("spinosa")])).toBe(true)
