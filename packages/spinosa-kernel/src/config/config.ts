...
import { InstallationLocal, InstallationVersion } from "@spinosa/kernel-core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@spinosa/kernel-core/v1/config/console-state"
import { FSUtil } from "@spinosa/kernel-core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@spinosa/kernel-core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@spinosa/kernel-core/v1/config/config"
import { RemoteAuthError } from "@spinosa/kernel-core/v1/config/error"
import { ConfigPermissionV1 } from "@spinosa/kernel-core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["spinosa.jsonc", "spinosa.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SPINOSA_CONFIG && !Flag.SPINOSA_CONFIG_DIR && !Flag.SPINOSA_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "spinosa.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "spinosa.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SPINOSA_CONFIG_CONTENT") return "local"
    ...
    const wellknownURL = `${url}/.well-known/spinosa`
        ...
        if (Flag.SPINOSA_CONFIG) {
          yield* merge(Flag.SPINOSA_CONFIG, yield* loadFile(Flag.SPINOSA_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SPINOSA_CONFIG })
        }

        if (!Flag.SPINOSA_DISABLE_PROJECT_CONFIG) {
  for (const file of yield* ConfigPaths.files("spinosa", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SPINOSA_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SPINOSA_CONFIG_DIR", { path: Flag.SPINOSA_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.spinosa/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (Flag.SPINOSA_CONFIG_CONTENT) {
          ...
          yield* Effect.logDebug("loaded custom config from SPINOSA_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["SPINOSA_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("SPINOSA_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SPINOSA_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.SPINOSA_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("SPINOSA_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.SPINOSA_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.SPINOSA_DISABLE_PRUNE) {
