...
- Host `darwin-arm64` strict smoke (`internal smoke provider-catalog` / `version` / `doctor` / `internal smoke native-imports`) is fail-closed: any smoke failure fails the release. `version`/`doctor` never dlopen the TUI natives; `native-imports` loads OpenTUI, FFF, watcher, node-pty, and canvas without starting an interactive UI. `provider-catalog` runs first on a fresh HOME so it exercises the embedded models.dev snapshot path (never the network) and proves the `/provider` list transform yields a non-empty catalog — the empty connect-dialog outage shipped green because no gate touched the catalog.
