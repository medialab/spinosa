...
import type * as SDK from "@spinosa/sdk/v2"
...
import { SessionShareTable } from "@spinosa/kernel-core/share/sql"
...
const disabled = process.env["SPINOSA_DISABLE_SHARE"] === "true" || process.env["SPINOSA_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/ShareNext") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const full = Effect.fn("ShareNext.full")(function* (sessionID: SessionID) {
      ...
      const diffOption = yield* session.diff(sessionID)
      const diffs = Option.getOrElse(diffOption, () => [] as SDK.SnapshotFileDiff[])
