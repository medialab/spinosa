...
import type { AgentPartInput, FilePartInput, OpencodeClient, Session, TextPartInput } from "@spinosa/sdk/v2/client"
import type {
  Project,
  ProjectCurrent,
  SessionApi,
...
export function createCompatibleApi(input: CompatibleInput): CompatibleApi {
  ...
  // Shape reads the V1 shim first (it defines integration/file.find and the
  // other adapted namespaces) and falls back to the live client for
  // pass-through namespaces. The adapted V2 client alone cannot serve as the
  // shape: it lacks the legacy `integration` root and V1-only methods.
  const shape = new Proxy(v1, {
    get(target, property) {
