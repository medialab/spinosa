...
import { AbsolutePath, type DeepMutable } from "@spinosa/kernel-core/schema"
import { ProviderV2 } from "@spinosa/kernel-core/provider"
import { ModelV2 } from "@spinosa/kernel-core/model"
import { LocationServiceMap, locationServiceMapLayer } from "@spinosa/kernel-core/location-services"
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/Agent") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Agent.state")(function* (ctx) {
        ...
        const referenceDirs = Object.keys(cfg.references ?? cfg.reference ?? {}).length
          ...
          : []
        const envWhitelist = (process.env.SPINOSA_WHITELISTED_DIRECTORIES ?? "")
          .split(",")
          .map((d) => d.trim())
          .filter(Boolean)
          .map((d) => path.join(d, "*"))
        ...
        const agents: Record<string, Info> = {
          build: {
            name: "build",
            description: "Orchestrator-Editor. The default agent for editing and execution tasks.",
            options: {},
          ...
          plan: {
            name: "plan",
            description: "Orchestrator-Planner. Planning mode that disallows all edit tools.",
            options: {},
            permission: Permission.merge(
              defaults,
              Permission.fromConfig({
                ...
                edit: {
                  "*": "deny",
                  [path.join(".spinosa", "plans", "*.md")]: "allow",
        ...
        const defaultInfo = Effect.fnUntraced(function* () {
          ...
          if (build && build.mode !== "subagent" && build.hidden !== true) return build
