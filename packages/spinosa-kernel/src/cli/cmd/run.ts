import type { PermissionV1 } from "@spinosa/kernel-core/v1/permission"
...
import { createSpinosaClient, type OpencodeClient, type ToolPart } from "@spinosa/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run spinosa with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running spinosa server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to SPINOSA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SPINOSA_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
