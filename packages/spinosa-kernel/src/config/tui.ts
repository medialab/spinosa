...
import { isRecord } from "@spinosa/tui/util/record"
...
import { TuiKeybind } from "@spinosa/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old spinosa.json shape) still get their settings applied.
  ...
  // 2. Explicit SPINOSA_TUI_CONFIG override, if set.
  if (Flag.SPINOSA_TUI_CONFIG) {
    const configFile = Flag.SPINOSA_TUI_CONFIG
  ...
  // 4. `.spinosa` directories (and SPINOSA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".spinosa") || dir === Flag.SPINOSA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".spinosa") && dir !== Flag.SPINOSA_CONFIG_DIR) continue
