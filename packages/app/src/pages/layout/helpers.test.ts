...
import { type Session } from "@spinosa/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("spinosa://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("spinosa://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("spinosa://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("spinosa://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("spinosa://open-project")).toBeUndefined()
    expect(parseDeepLink("spinosa://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "spinosa://open-project?directory=/a",
      "spinosa://other?directory=/b",
      "spinosa://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("spinosa://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("spinosa://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("spinosa://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("spinosa://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "spinosa://new-session?directory=/a",
      "spinosa://open-project?directory=/b",
      "spinosa://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SPINOSA__: {
        deepLinks: ["spinosa://open-project?directory=/a"],
      },
    } as unknown as Window & { __SPINOSA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["spinosa://open-project?directory=/a"])
