...
async function startListener() {
  Flag.SPINOSA_SERVER_PASSWORD = auth.password
  Flag.SPINOSA_SERVER_USERNAME = auth.username
  process.env.SPINOSA_SERVER_PASSWORD = auth.password
  process.env.SPINOSA_SERVER_USERNAME = auth.username
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.SPINOSA_DISABLE_DEFAULT_PLUGINS
    process.env.SPINOSA_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.SPINOSA_DISABLE_DEFAULT_PLUGINS
      else process.env.SPINOSA_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
