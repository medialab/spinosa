...
const context = createContext<{
  ...
  toolCalloutSide: (callID: string) => ToolCalloutInfo | undefined
  sessionID: string
  conceal: () => boolean
            ...
            >
              ...
              <box flexShrink={0}>
                ...
                <Show when={visible()}>
                  ...
                  >
                    <box
                      paddingLeft={promptPadding()}
                      paddingRight={promptPadding()}
                    >
                      <Prompt
                        visible={visible()}
                        ref={bind}
...
function UserMessage(props: {
  ...
  return (
    <>
      <Show when={text()}>
        <TranscriptRow id={props.message.id}>
          <box
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage; gapBefore?: number }) {
  ...
  const callout = createMemo(() => {
    const info = ctx.toolCalloutSide(props.part.callID)
    if (!info) return
    const toolOffset = toolUsesBlockLayout(
      props.part.tool,
      toolprops.metadata,
      toolprops.output,
      ctx.showGenericToolOutput(),
    )
      ? 1
      : 0
    return {
...
export function toolUsesBlockLayout(
  tool: string,
  ...
  const display = toolDisplay(tool)
  if (display === "bash") return stringValue(metadata.output) !== undefined
  if (display === "write") return metadata.diagnostics !== undefined
  if (display === "edit") return stringValue(metadata.diff) !== undefined
  if (display === "apply_patch") return parseApplyPatchFiles(metadata.files).length > 0
...
export function buildToolCalloutSummary(
  tool: string,
  ...
  if (display === "bash") {
    return { tag: "SHELL", command: stringValue(inputValue.command) ?? "Run command" }
  }
  if (display === "glob") {
    const inPath = stringValue(inputValue.path) ? ` in ${formatPath(stringValue(inputValue.path))}` : ""
  ...
  if (display === "grep") {
    const inPath = stringValue(inputValue.path) ? ` in ${formatPath(stringValue(inputValue.path))}` : ""
    return { tag: "GREP", command: `"${stringValue(inputValue.pattern) ?? ""}"${inPath}`.trim() }
  }
  if (display === "webfetch") {
    return { tag: "FETCH", command: stringValue(inputValue.url) ?? "Fetch" }
  }
  if (display === "websearch") {
    return {
  ...
  if (display === "todowrite") {
    return { tag: "TODO", command: "Update plan" }
  }
  if (display === "question") {
    const count = parseQuestions(inputValue.questions).length
    return { tag: "QUESTION", command: `Ask ${count || 1} question${count === 1 ? "" : "s"}` }
  }
  if (display === "skill") {
  ...
  return {
    tag: tool.toUpperCase(),
    command: `${tool} ${input(inputValue)}`.trim(),
  }
}
