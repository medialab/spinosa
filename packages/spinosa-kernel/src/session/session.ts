...
import { Slug } from "@spinosa/kernel-core/util/slug"
...
import type { ProviderMetadata, Usage } from "@spinosa/llm"
...
import { PartTable, SessionTable } from "@spinosa/kernel-core/session/sql"
...
import { NonNegativeInt, optional } from "@spinosa/kernel-core/schema"
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".spinosa", "plans")
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/Session") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const writeSessionFile = (session: Info) =>
      Effect.gen(function* () {
        const dir = path.join(session.directory, ".spinosa", "sessions")
