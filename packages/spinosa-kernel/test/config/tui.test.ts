...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.SPINOSA_CONFIG
  delete process.env.SPINOSA_TUI_CONFIG
...
it.instance("project config takes precedence over SPINOSA_TUI_CONFIG (matches SPINOSA_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("SPINOSA_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from SPINOSA_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
