...
describe("escConfirmStop", () => {
  test("unarmed states never confirm", () => {
    expect(escConfirmStop(0, undefined, "session:one", 1_000)).toBe(false)
    expect(escConfirmStop(undefined, undefined, "session:one", 1_000)).toBe(false)
  })

  test("second press inside 10s confirms the stop", () => {
    expect(escConfirmStop(1_000, "session:one", "session:one", 1_000 + ESC_ARM_WINDOW_MS - 1)).toBe(true)
  ...
  test("an arm cannot cross from evaluation into the conversation turn", () => {
    expect(escConfirmStop(1_000, "session:one", "session:two", 1_001)).toBe(false)
  })
