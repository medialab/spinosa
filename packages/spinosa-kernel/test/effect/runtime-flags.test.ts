...
describe("RuntimeFlags", () => {
      ...
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            ...
            SPINOSA_ENABLE_EXA: "true",
            SPINOSA_ENABLE_PARALLEL: "true",
            SPINOSA_ENABLE_EXPERIMENTAL_MODELS: "true",
            SPINOSA_ENABLE_QUESTION_TOOL: "true",
  ...
  it.effect("layer parses SPINOSA_EXPERIMENTAL_LSP_TY", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            SPINOSA_EXPERIMENTAL_LSP_TY: "true",
          }),
        ),
      )
  ...
  it.effect("enables native LLM via dedicated flag only", () =>
    Effect.gen(function* () {
      const explicit = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL_NATIVE_LLM: "true" })))
      const umbrella = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL: "true" })))
  ...
  it.effect("enables WebSockets via dedicated flag only", () =>
    Effect.gen(function* () {
      const explicit = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL_WEBSOCKETS: "true" })))
      const umbrella = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL: "true" })))
  ...
  it.effect("disableExternalSkills reads SPINOSA_DISABLE_EXTERNAL_SKILLS", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_EXTERNAL_SKILLS: "true" })))
  ...
  it.effect("disableLspDownload reads SPINOSA_DISABLE_LSP_DOWNLOAD", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_LSP_DOWNLOAD: "true" })))
  ...
  it.effect("disableClaudeCodePrompt reads SPINOSA_DISABLE_CLAUDE_CODE_PROMPT", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_CLAUDE_CODE_PROMPT: "true" })))

      expect(flags.disableClaudeCodePrompt).toBe(true)
    }),
  )

  it.effect("disableClaudeCodePrompt inherits SPINOSA_DISABLE_CLAUDE_CODE", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_CLAUDE_CODE: "true" })))

      expect(flags.disableClaudeCodePrompt).toBe(true)
    }),
  )

  it.effect("experimentalIconDiscovery reads SPINOSA_EXPERIMENTAL_ICON_DISCOVERY", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL_ICON_DISCOVERY: "true" })))

      expect(flags.experimentalIconDiscovery).toBe(true)
    }),
  )

  it.effect("experimentalIconDiscovery inherits SPINOSA_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_EXPERIMENTAL: "true" })))

      expect(flags.experimentalIconDiscovery).toBe(true)
    }),
  )

  it.effect("specific experimental flags override SPINOSA_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            SPINOSA_EXPERIMENTAL: "true",
            SPINOSA_EXPERIMENTAL_ICON_DISCOVERY: "false",
          }),
        ),
      )
  ...
  it.effect("experimentalOxfmt is enabled by SPINOSA_EXPERIMENTAL_OXFMT", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            SPINOSA_EXPERIMENTAL_OXFMT: "true",
          }),
        ),
      )
  ...
  it.effect("experimentalOxfmt inherits SPINOSA_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
  ...
  for (const input of [
    { name: "absent", config: {}, expected: undefined },
    {
      ...
      config: { SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "1234" },
      expected: 1234,
    },
    {
      name: "invalid string",
      config: { SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "nope" },
      expected: undefined,
    },
    { name: "zero", config: { SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "0" }, expected: undefined },
    { name: "negative", config: { SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "-1" }, expected: undefined },
    {
      name: "non-integer",
      config: { SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "1.5" },
      expected: undefined,
    },
  ]) {
  ...
  for (const input of [
    ...
    {
      name: "valid positive integer",
      config: { SPINOSA_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "1234" },
      expected: 1234,
    },
    {
      name: "invalid string",
      config: { SPINOSA_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "nope" },
      expected: undefined,
    },
    { name: "zero", config: { SPINOSA_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "0" }, expected: undefined },
    { name: "negative", config: { SPINOSA_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "-1" }, expected: undefined },
    {
      name: "non-integer",
      config: { SPINOSA_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "1.5" },
      expected: undefined,
    },
  ]) {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromUnknown({
              ...
              SPINOSA_EXPERIMENTAL: "true",
              SPINOSA_ENABLE_EXA: "true",
              SPINOSA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "1234",
              SPINOSA_CLIENT: "desktop",
            }),
          ),
        ),
  ...
  it.effect("disableClaudeCodeSkills reads SPINOSA_DISABLE_CLAUDE_CODE_SKILLS", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_CLAUDE_CODE_SKILLS: "true" })))

      expect(flags.disableClaudeCodeSkills).toBe(true)
    }),
  )

  it.effect("disableClaudeCodeSkills inherits SPINOSA_DISABLE_CLAUDE_CODE", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ SPINOSA_DISABLE_CLAUDE_CODE: "true" })))
