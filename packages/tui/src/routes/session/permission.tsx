...
import { SPINOSA_BASE_MODE, useBindings, useCommandShortcut } from "../../keymap"
...
function replyPermission(
  ...
  input: { reply: "once" | "always" | "reject"; message?: string; directory?: string; workspace?: string },
) {
  const isV2 = request.metadata?.__v2 === true
  if (isV2) {
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  return (
    <box flexDirection="column" gap={1}>
      ...
      <Show when={!diff()}>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>No change preview is available.</text>
        </box>
      </Show>
    </box>
  )
      ...
      <Match when={store.stage === "always"}>
        <Prompt
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until Spinosa is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until Spinosa is restarted</text>
                  <box>
          ...
          onSelect={(option) => {
            ...
            void replyPermission(sdk, props.request, {
              reply: "always",
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "external_directory") {
              ...
              return {
                icon: "←",
                title: `Access external folder ${dir}`,
                body: (
            ...
            if (permission === "doom_loop") {
              return {
                icon: "⟳",
                title: "Keep trying after repeated failures",
                body: (
                  <box paddingLeft={1}>
          ...
          const body = (
            <Prompt
              ...
              header={header()}
              body={current.body}
              options={{ once: "Allow this time", always: "Allow for session", reject: "Don’t allow" }}
              escapeKey="reject"
              fullscreen
              onSelect={(option) => {
                if (option === "always") {
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <box flexDirection="row" gap={1} paddingLeft={1}>
          ...
          <text fg={theme.text}>Don’t allow</text>
        </box>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Spinosa what to do differently</text>
        </box>
      </box>
      <box
