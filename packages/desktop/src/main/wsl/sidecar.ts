...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export SPINOSA_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SPINOSA_CLIENT=desktop",
    `export SPINOSA_SERVER_USERNAME=${shellEscape(username)}`,
    `export SPINOSA_SERVER_PASSWORD=${shellEscape(password)}`,
