...
const DEFAULT_SERVER_URL_KEY = "spinosa.settings.dat:defaultServerUrl"
...
const getCurrentUrl = () => {
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_SPINOSA_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_SPINOSA_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.SPINOSA_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
