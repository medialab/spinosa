...
import "./styles.css"
import { Splash } from "@spinosa/ui/logo"
import { useTheme } from "@spinosa/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.SPINOSA_CHANNEL === "prod" &&
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__SPINOSA__ ??= {}
  const pending = window.__SPINOSA__.deepLinks ?? []
  window.__SPINOSA__.deepLinks = [...pending, ...urls]
...
// Launch always lands on the Spinosa home (/). Route restore is
// intentionally disabled: every window starts at the home screen with the
// workspace actions, like the TUI global home.
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("spinosa.global.dat").getItem("language")
        ...
        <Show when={effectiveDefaultServer()} keyed>
          {(key) => (
            <AppInterface
              ...
              serverScoped={
                <DesktopFirstLaunchOnboarding initialUrl="/" onLoaded={onboarding.resolve} />
              }
            >
              <Inner />
            </AppInterface>
