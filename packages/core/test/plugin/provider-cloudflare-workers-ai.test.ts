...
import { CloudflareWorkersAIPlugin } from "@spinosa/kernel-core/plugin/provider/cloudflare-workers-ai"
...
describe("CloudflareWorkersAIPlugin", () => {
  ...
  it.effect("uses env API key over auth or configured API key and keeps the Cloudflare User-Agent", () =>
    ...
    expect(headers["user-agent"]).toMatch(/^spinosa\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//)
      }),
    ),
  )
