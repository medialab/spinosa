...
export async function checkServerHealth(
  ...
  const attempt = async (count: number): Promise<ServerHealth> => {
    const current = await createSdkForServer({ server, fetch })
      .v2.health.get({ signal })
      .then((x) => (x.data?.healthy === true ? { data: { healthy: true as const } } : null))
      .catch(() => null)
    if (current?.data) {
      // /api/health carries no version; read it best-effort from the legacy endpoint.
      const version = await createSdkForServer({ server, fetch })
        .global.health({ signal })
        .then((x) => (x.error ? undefined : x.data?.version))
        .catch(() => undefined)
      return { healthy: true, version }
    }
    if (signal?.aborted) return { healthy: false }

    return createSdkForServer({ server, fetch })
      .global.health({ signal })
