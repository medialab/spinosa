...
import { define } from "@spinosa/plugin/v2/effect"
...
import { LocationServiceMap } from "@spinosa/kernel-core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "spinosa.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "spinosa.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
