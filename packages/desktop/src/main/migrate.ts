...
// `opencode.settings.dat` is special: it maps to the `spinosa.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"spinosa.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opencode.settings.dat → the electron settings store ("spinosa.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "spinosa.global.dat").
  const storeName = filename === "opencode.settings.dat" ? "spinosa.settings" : filename
