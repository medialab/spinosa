...
description: |
  Searches raw corpus, maps, and dictionary for evidence. Writes evidence packets to files; returns file paths to orchestrator. 
tools: Read, Grep, Glob, jev
skills:
  - spinosa-searcher
---
...
4. **If continuing**: broaden the search — try stems, synonyms, drop the most specific term.
5. **When stopped**: write the evidence packet with what was found. Record `search_termination: [reason]` in the frontmatter.
