  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-opencode", distro: name, startedAt: Date.now() }, async () => {
        // Spinosa follow-up: WSL distros need the Spinosa installer + `spinosa
        // serve` wiring (runtime.ts/startup.ts still target opencode). Refuse
        // instead of installing the wrong product into the distro.
        throw new Error(nativeT("desktop.wsl.error.unavailable"))
      })
    },
